...
This feature implements a simple Twitch client handler for the VibeGuide backend service. The handler will manage Twitch API authentication and provide methods for fetching top streams and popular game categories through REST endpoints.
...
- **API_Handler**: HTTP request handler that exposes top streams and categories functionality via REST endpoints
- **Game_Category**: Twitch game category containing game information and metadata
...
**User Story:** As an API consumer, I want a REST endpoint for popular categories, so that I can access Twitch game categories data through HTTP requests.

#### Acceptance Criteria

1. THE API_Handler SHALL provide an endpoint for fetching popular game categories
2. THE API_Handler SHALL return categories data in JSON format with game information
3. THE API_Handler SHALL support configurable limits for the number of categories returned
4. IF the Twitch API request fails, THEN THE API_Handler SHALL return appropriate HTTP error responses
